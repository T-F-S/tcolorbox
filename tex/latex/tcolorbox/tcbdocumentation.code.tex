...
\tcbset{
  ...
  doc key width/.store in=\kvtcb@doc@keywidth,
  doc key ratio/.style={doc key width=#1\columnwidth},
...
\NewDocumentCommand \docKeyChoices { s m }
  {
    \dim_compare:nNnF \l__tcobox_doc_key_wd_dim > { 0pt }
      {
        \tcb@warning{Negative~width~for~'docKeyChoices'.~Enlarge~'doc~key~ratio'!}
      }
...
\cs_new:Npn \__tcobox_doc_head_key:
  {
    ...
    \dim_set:Nn \l__tcobox_doc_key_wd_dim
      { \kvtcb@doc@keywidth - \box_wd:N \l__tcobox_tmpa_box }
