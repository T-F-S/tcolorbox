...
\tcbset{%
  ...
  attach@title@boxed/.style n args={6}{%
    ...
    code={\def\tcb@specialtitle@hook{%
      ...
      \or%
        \tcbdimto\tcb@xshift@boxedtitle{\kvtcb@xshift@boxedtitle+\kvtcb@left@rule+\kvtcb@boxsep+\kvtcb@leftupper-\tcb@xshift@boxedtitle@delta}%
      \else%
        \tcbdimto\tcb@xshift@boxedtitle{\kvtcb@xshift@boxedtitle+\tcb@xshift@boxedtitle@delta-(\kvtcb@right@rule+\kvtcb@boxsep+\kvtcb@rightupper)}%
      \fi%
...
\cs_new_protected_nopar:Npn \__tcobox_include_graphics:
  {
        ...
        \legacy_if:nTF { tcb@fixedheight }
          {
            \begin{ tcolorbox }
              \__tcobox_include_graphics:en
                { \l__tcobox_graphics_options_tl,width=\linewidth,height=\tcbtextheight,keepaspectratio }
                { \imagepath }
            \end{ tcolorbox }
          }
          {
                    ...
                    underlay=
                      {
                        ...
                        \path
                          [
                            fill~zoom~image* = { \l__tcobox_graphics_options_tl } { \imagepath }
                          ]
                          ([xshift=\kvtcb@boxsep+\kvtcb@leftupper,yshift=\kvtcb@boxsep+\kvtcb@bottom]
                            interior.south~west)~rectangle~
                          ([xshift=-\kvtcb@boxsep-\kvtcb@rightupper,yshift=-\kvtcb@boxsep-\kvtcb@top]
                            interior.north~east);
                        \end{ tcbclipinterior }
                      }
                  ]
...
\cs_new_protected:Npn \__tcobox_include_graphics_preparefile:n #1
  {
...
\tl_new:N \l__tcobox_beamer_temporal_args_tl
\tl_new:N \l__tcobox_beamer_alt_args_tl

\tcbset{
  only    /.code = { \__tcobox_beamer_only:w #1 },
  hide    /.code = { \__tcobox_beamer_hide:w #1 },
  alert   /.code = { \__tcobox_beamer_alert:w #1 },
  alt     /.code = { \__tcobox_beamer_alt:w #1 },
  temporal/.code = { \__tcobox_beamer_temporal:w #1 },
  set~alt /.code =
    {
      \tl_set:Nn \l__tcobox_beamer_alt_args_tl { #1 }
    },
  set~alt = {} {},
  use~alt /.code =
    {
      \use:e
        {
          \__tcobox_beamer_alt:w \exp_not:n { #1 } \exp_not:o \l__tcobox_beamer_alt_args_tl
        }
    },
  set~temporal /.code =
    {
      \tl_set:Nn \l__tcobox_beamer_temporal_args_tl { #1 }
    },
  set~temporal = {} {} {},
  use~temporal /.code =
    {
      \use:e
        {
          \__tcobox_beamer_temporal:w \exp_not:n { #1 } \exp_not:o \l__tcobox_beamer_temporal_args_tl
        }
    },
}

\cs_new_protected:Npn \__tcobox_beamer_only:w <#1>#2
  {
    \only <#1> { \tcbset{#2} }
  }

\cs_new_protected:Npn \__tcobox_beamer_hide:w <#1>
  {
    \only <#1> { \tcbset{ beamer~hidden } }
  }

\cs_new_protected:Npn \__tcobox_beamer_alert:w <#1>
  {
    \only <#1> { \tcbset{ beamer~alerted } }
  }

\cs_new_protected:Npn \__tcobox_beamer_alt:w <#1>#2#3
  {
    \alt <#1> { \tcbset{#2} } { \tcbset{#3} }
  }

\cs_new_protected:Npn \__tcobox_beamer_temporal:w <#1>#2#3#4
  {
    \temporal <#1> { \tcbset{#2} } { \tcbset{#3} } { \tcbset{#4} }
  }
...
\tcbset{%
  reset@skins/.style={%
    ...
    boxed title size=title,no boxed title style,hbox boxed title,code={\let\tcb@specialgeonodes@hook\@empty},%
